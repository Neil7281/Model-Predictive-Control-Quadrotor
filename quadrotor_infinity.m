...
function quadrotor_6DOF_control()
    ...
    global w1_des_history w2_des_history w3_des_history w4_des_history t_history;
    w1_des_history = [];
    w2_des_history = [];
    w3_des_history = [];
    w4_des_history = [];
  ...
  if t <= 20
    ...
    hold on 
    %plot3(xref,yref,zdesired,'LineWidth',3);
    hold off
...
fprintf('IntegraI Time Absolute Error (ITAE): %.4f\n', ITAE);
    ...
    hold on 
    plot(t, xref,"--",'LineWidth', 4);legend("Actual","Desired")
    hold off
    subplot(3, 1, 2); plot(t, y,'LineWidth', 4); xlabel('Time (s)'); ylabel('Y  (m)'); title('Y Position');
    hold on 
    plot(t,yref,"--",'LineWidth', 4);
    hold off
    subplot(3, 1, 3); plot(t, z,'LineWidth', 4); xlabel('Time (s)'); ylabel('Z (m)'); title('Z Position');ylim([0 12]);
    hold on 
  ...
  grid on;
    ...
    figure;
    subplot(3, 1, 1); plot(t, rad2deg(phi)); xlabel('Time (s)'); ylabel('Roll \phi (deg)'); title('Roll Angle');
    hold on
    subplot(3, 1, 1); plot(t, rad2deg(phi_ref),"--"); xlabel('Time (s)'); ylabel('Roll \phi (deg)'); title('Roll Angle');
    hold off
    subplot(3, 1, 2); plot(t, rad2deg(theta)); xlabel('Time (s)'); ylabel('Pitch \theta (deg)'); title('Pitch Angle');
    hold on 
    subplot(3, 1, 2); plot(t, rad2deg(theta_ref),"--"); xlabel('Time (s)'); ylabel('Pitch \theta (deg)'); title('Pitch Angle');
    hold off
    subplot(3, 1, 3); plot(t, rad2deg(psi),'LineWidth', 4); xlabel('Time (s)'); ylabel('Yaw \psi (deg)'); title('Yaw Angle');
    hold on
    subplot(3, 1, 3); plot(t, psi_ref,"--",'LineWidth', 4); xlabel('Time (s)'); ylabel('Yaw \psi (deg)'); title('Yaw Angle');
    hold off
end
...
function dXdt = quadrotor_dynamics(t, X)
 global w1_des_history w2_des_history w3_des_history w4_des_history t_history;
   ...
   xref = 3*sin(0.2*t);
    ...
    psi_ref = 0;
    xref_dot =  0.6*cos(0.2*t);
    yref_dot = 0.3*cos(0.1*t);
    xref_ddot = -0.12*sin(0.2*t);                                          % 
    yref_ddot = -0.03*sin(0.1*t);
    xref_dddot = -0.024*cos(0.2*t);
    yref_dddot = -0.003*cos(0.1*t);
    zdesired_dot = 0;
   % Control Gains for PID
k_p_pos_z = 163.0297; k_p_ang_phi = 380.8515;
k_d_pos_z = 120.0335; k_d_ang_phi = 150.4867;
k_p_pos_y = 0.1000; k_p_ang_theta = 400.4488;
k_d_pos_y = 0.1000; k_d_ang_theta = 100.0240;
k_p_pos_x = 0.1300; k_p_ang_psi = 100.1353;
k_d_pos_x = 0.15004; k_d_ang_psi = 280.7893;
   ...
   s_ref_dot = I(3,:);
    ...
    w4_des =  w_h + w_lift - w_phi - w_theta + w_psi;  
   % Apply saturation limits to rotor speeds
