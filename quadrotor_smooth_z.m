...
function quadrotor_6DOF_control()
    ...
    tspan = [0:0.01:100]; % Time span for simulation (seconds)
   ...
   % [t, states] = ode45(@quadrotor_dynamics, tspan, initial_conditions);
    ...
    phi = states(:, 7);
    theta = states(:, 8); psi = states(:, 9);phi_ref = states(:,13);theta_ref = states(:,14);
...
% Define the smooth trajectory equation
  if t <= 20
    zdesired = (t.^3 .* (3*t.^2 - 150*t + 2000)) / 160000;
    zdesired_dot =  (1/160000) * (15*t.^4 - 600*t.^3 + 6000*t.^2);
  else 
    zdesired = 10;
    zdesired_dot = 0;
  end 
    ...
    plot3(x,y,z,'--','LineWidth',6);%xlabel("X Position");ylabel('Y Position');zlabel('Z Position');
    hold on 
    grid on;
...
% Position tracking error over time
    %plot3(xref,yref,zdesired,"--",'LineWidth',1,'Color','blue');
    hold off
    figure;
    ...
    subplot(3, 1, 1); plot(t,x,'LineWidth', 1); xlabel('Time (s)'); ylabel('X (m)'); title('X Position');
    hold on 
    plot(t, xref,"--",'LineWidth', 1);
    hold off
    subplot(3, 1, 2); plot(t, y,'LineWidth', 1); xlabel('Time (s)'); ylabel('Y  (m)'); title('Y Position');
    hold on 
    plot(t,yref,"--",'LineWidth', 1);
    hold off
    subplot(3, 1, 3); plot(t, z,'LineWidth', 1); xlabel('Time (s)'); ylabel('Z (m)'); title('Z Position');ylim([0 12]);
    hold on 
    plot(t,zdesired.*ones(size(t)),"--",'LineWidth', 1);
    hold off
    
    % Plot the Euler angles over time
    figure;
    plot(t_history,phi_ref_history); xlabel('Time (s)'); ylabel('Roll \phi'); title('Roll and Pitch Angle');
    hold on 
    plot(t_history,theta_ref_history);legend("phi ref", "theta ref");
    hold off

    %figure;
    %subplot(2,1,1);plot(t_history,phi_ref_history - phi);xlabel('Time (s)'); ylabel('error (m)'); title('phi error');
    %subplot(2,1,2);plot(t_history,theta_ref_history - theta);xlabel('Time (s)'); ylabel('error (m)'); title('theta error');

  figure;
  plot(t_history, w1_des_history, 'LineWidth', 1);
  hold on 
  plot(t_history, w2_des_history, '--','LineWidth', 1);
  plot(t_history, w3_des_history, ':', 'LineWidth', 1,'Color','green');
  plot(t_history, w4_des_history, '-.','LineWidth', 1);legend('$w_1$','$w_2$','$w_3$','$w_4$','Interpreter', 'latex');
  hold off
  xlabel('Time (s)');
  ylabel('$w$ (rad/s)','Interpreter','latex');
  title('Desired Motor Speeds vs Time');
  grid on;
...
% Define the smooth trajectory equation
  if t <= 20
    ...
    %zdesired =  c5 * t.^5 + c4 * t.^4 + c3 * t.^3;
    %zdesired_dot = 5*c5*t^4 + 4*c4*t^3 + 3*c3*t^2;
    %zdesired = 10; 
    xref = 3*cos(0.1*t);
    yref = 3*sin(0.1*t);
  ...
  k_p_pos_z = 193.0297; k_p_ang_phi = 280.8515;
  k_d_pos_z = 120.0335; k_d_ang_phi = 40.4867;
  k_p_pos_y = 0.1000; k_p_ang_theta = 430.4488;
  k_d_pos_y = 0.7000; k_d_ang_theta = 100.0240;
  k_p_pos_x = 0.1200; k_p_ang_psi = 100.1353;
  k_d_pos_x = 0.1004; k_d_ang_psi = 80.7893;
   ...
   s_ref_dot = I(3,:);
    ...
    w_psi = k_p_ang_psi*(psi_ref - psi) + k_d_ang_psi*(0 - s_dot);
    w_h = sqrt((m*g)/(4*k_f));
    ...
    w_des_history = [w_des_history,w_des];
    w1_des = min(max(w1_des, 0), 1500);
    w2_des = min(max(w2_des, 0), 1500);
    w3_des = min(max(w3_des, 0), 1500);
    w4_des = min(max(w4_des, 0), 1500);
    ...
    w = [t_history, w1_des_history,w2_des_history, w3_des_history,  w4_des_history];
