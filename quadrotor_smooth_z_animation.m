...
% Define the smooth trajectory equation
  if t <= 20
    ...
    ylabel('Y Position (m)');
    zlabel('Z Position (m)');
    title('3D Trajectory of Quadrotor');
    %axis equal;
    %axis([-5 5 -5 5 0 20]);
    view(3);

    % Plot the reference trajectory
    plot3(xref, yref, zdesired, '--', 'LineWidth', 1.5);

    % Initialize plot objects for animation
    quadrotor_path = plot3(nan, nan, nan, 'b-', 'LineWidth', 2); % Path
    quadrotor_position = plot3(nan, nan, nan, 'ro', 'MarkerSize', 8, 'MarkerFaceColor', 'r'); % Position

    % Animation loop
   N = 200;  % Target number of frames
indices = round(linspace(1, length(t), N));
for k = indices
    set(quadrotor_path, 'XData', x(1:k), 'YData', y(1:k), 'ZData', z(1:k));
    set(quadrotor_position, 'XData', x(k), 'YData', y(k), 'ZData', z(k));
    pause(7 / N);  % Ensures the entire animation completes in 5 seconds
end
    hold off;
end
...
% Define the smooth trajectory equation
  ...
  k_p_pos_z = 193.0297; k_p_ang_phi = 380.8515;
  ...
  k_d_pos_x = 0.3004; k_d_ang_psi = 280.7893;
    ...
    w_phi = k_p_ang_phi*(phi_ref - phi) + k_d_ang_phi*(phi_ref_dot - phi_dot);
    w_theta = k_p_ang_theta*(theta_ref - theta) + k_d_ang_theta*(theta_ref_dot - theta_dot);
    w_psi = k_p_ang_psi*(psi_ref - psi) + k_d_ang_psi*(0 - psi_dot);
    w_h = sqrt((m*g)/(4*k_f));
